...
"""
Two-model debate entrypoint: uses ``RayPPODebateTrainer`` (see ``ray_trainer_debate.py``).

Hydra config must define ``debate.pool_b`` and ``debate.actor_rollout_ref_b`` (see module docstring there).
"""
...
from verl.trainer.ppo.ray_trainer_debate import RayPPODebateTrainer
...
def _resolve_rl_parquet_paths(config_data, original_cwd: str) -> None:
    """Make train/val parquet paths absolute using Hydra launch directory (relative paths are common)."""
    ...
    def resolve_one(path: str) -> str:
        ...
        return os.path.normpath(os.path.join(original_cwd, path))

    with open_dict(config_data):
        for key in ("train_files", "val_files"):
            if key not in config_data or config_data[key] is None:
                continue
            val = config_data[key]
            if isinstance(val, (list, ListConfig)):
                config_data[key] = [resolve_one(x) for x in val]
            else:
                config_data[key] = resolve_one(val)


def _require_rl_parquet_files(config_data) -> None:
    """Fail fast with a clear message when parquet paths are missing."""
    ...
    if missing:
        ...
        raise FileNotFoundError(
            "RLHF dataset parquet file(s) not found:\n  "
            f"{joined}\n\n"
            "From the repository root, build GSM8K parquets for example:\n"
            "  python examples/data_preprocess/gsm8k.py --local_save_dir data/gsm8k\n"
            "Or set GSM8K_TRAIN / GSM8K_TEST, or override data.train_files / data.val_files."
        )
...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        if use_legacy_worker_impl == "disable":
            ...
            self.role_worker_mapping[DebateRole.ActorRolloutB] = self.role_worker_mapping[role]
            self.mapping[DebateRole.ActorRolloutB] = "debate_pool_b"
        ...
        self.role_worker_mapping[DebateRole.ActorRolloutB] = self.role_worker_mapping[Role.ActorRollout]
        self.mapping[DebateRole.ActorRolloutB] = "debate_pool_b"
        return actor_rollout_cls, ray_worker_group_cls
    ...
    def init_resource_pool_mgr(self, config):
        ...
        if not OmegaConf.select(config, "debate.pool_b"):
            raise ValueError("debate.pool_b.{n_gpus_per_node, nnodes} is required for main_ppo_debate")
        pb = config.debate.pool_b
        if pb.n_gpus_per_node <= 0 or pb.nnodes <= 0:
            raise ValueError("debate.pool_b.n_gpus_per_node and nnodes must be positive")
        resource_pool_spec["debate_pool_b"] = [pb.n_gpus_per_node] * pb.nnodes
    ...
    def run(self, config):
        ...
        try:
            ...
            _hydra_cwd = get_original_cwd()
        except Exception:
            _hydra_cwd = os.getcwd()
        _resolve_rl_parquet_paths(config.data, _hydra_cwd)
        _require_rl_parquet_files(config.data)

        if not OmegaConf.select(config, "debate.actor_rollout_ref_b"):
            raise ValueError("debate.actor_rollout_ref_b is required (full actor_rollout_ref-style subtree for model B)")
        ...
        local_path_b = copy_to_local(
            config.debate.actor_rollout_ref_b.model.path,
            use_shm=config.debate.actor_rollout_ref_b.model.get("use_shm", False),
        )
        ...
        tokenizer_b = hf_tokenizer(local_path_b, trust_remote_code=trust_remote_code)
        # Used for multimodal LLM, could be None
        processor = hf_processor(local_path, trust_remote_code=trust_remote_code, use_fast=True)
        processor_b = hf_processor(local_path_b, trust_remote_code=trust_remote_code, use_fast=True)
        ...
        # Initialize the debate PPO trainer (two actor groups, sync after party A).
        trainer = RayPPODebateTrainer(
            config=config,
            tokenizer=tokenizer,
